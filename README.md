...
combinations : 리스트의 모든 조합구하는 법

combinations(range(len(number)), 3) : 리스트 갯수에서 3개를 뽑아서 구함
...
bin(|) : 바이너리 함수, bin(a|b) 뜻은 a와 b의 이진수 중에 하나라도 1이면 1출력, 둘다 0이면 0출력

[2:] : binary 함수의 2번째 배열부터 시작

rjust: 오른쪽 공백을 매꿔줄 함수
